...
> parser (`core-parser/`) now reads every markdown file in `content/cv/**`,
> `content/projects/**`, and `content/freelance/**`. To regenerate the
> locale modules under `src/lib/locales/`, run `npm run sync-locales`. To
> regenerate the 4 public PDFs under `public/cv/`, run
> `npm run pdf:public` (the pre-push hook does this automatically). The
...
SEO/ATS keywords live inside the CV markdown as a `<!-- keywords: … -->`
comment (§1.3.2). The only remaining sidecar is the private **cv-special**
extras file (`content/special/{lang}-special.meta.yaml`); see §1.6.
...
1. When the portfolio is on a separate line, it uses the form
   `Portfolio · value` (only `value` is stored in `profile.portfolio`; the UI
...
- **Contacts**: parsed from the contact line(s) after the location line. The
  ...
  URL and stored in `profile.contacts` as a `Record<string, string>`. The parser
  only includes contacts that are present in the markdown.
- **Portfolio**: the value after `Portfolio ·` when that pattern appears; only
  the value is stored in `profile.portfolio`.
- **Headline**: the first `##` heading after the header block (after location,
  contacts, and portfolio lines).
- **Tagline (`profile.tagline`)** and **short tagline (`profile.taglineShort`)**:
  - **Two taglines in the markdown** (long + short): use HTML comments so the
    ...
    second line maps to `profile.taglineShort`.
  - **One tagline in the markdown**: do **not** use HTML comments for taglines.
    The single italic line after the headline maps to `profile.tagline`. Set
    `profile.taglineShort` to the empty string `""` (the key must remain
    present).
  - **No tagline in the markdown**: Set both `profile.tagline` and
    `profile.taglineShort` to an empty string `""` (the keys must remain
    present).
...
- **Education**: under `## Education` / `## Formazione`, each entry is one or
  ...
  expected layout).
- **Footer (`ParsedCv.footer`)**: the final block of the CV is a
  ...
  strips the wrapping quotes and stores the result at the **root** of the
  parser output as `footer` (not inside `profile`). This field is required —
  CVs without a footer line fail to parse.
- **Keywords (`ParsedCv.keywords`)**: an optional HTML comment anywhere in the
  file, `<!-- keywords: a, b, c -->`, supplies SEO/ATS keywords (comma-separated,
  trimmed). It is never rendered. The parser stores the list at the **root** of
  the output as `keywords`; absent comment → `[]`. This is the single source of
  truth for keywords — there is no keyword sidecar. Extraction lives in
  `readers/readMetadata.ts` behind a stable `Metadata` type.
...
(`export-pdf/templates/freelance.typ`), which inserts the Selected Projects
section between Education and the footer.

### 1.6 The cv-special sidecar (`*.meta.yaml`)

Keywords are no longer a sidecar — they live in the markdown as a
`<!-- keywords: … -->` comment (§1.3.2), and contact display labels are the
site's concern. The **only** remaining YAML sidecar is the private cv-special
extras file. It is owned by `export-pdf` (parsed in
`export-pdf/src/special/`), not by `core-parser` — the parser is yaml-free.
...
`pdf -- --template special --input content/cv/main/en-cv.md` will
...
```
content/cv/**/*.md            ──┐   core-parser/      → typed ParsedCv
content/projects/*.md          ─┤   (pure TS; reads     { profile, experiences,
content/freelance/*.md         ─┘   through a            education, projects,
                                    SourceResolver)      footer, keywords }
                                              │
                                              ├──→ export-pdf/  (Typst)  → PDFs
                                              │      ├─ public/cv/ (4)
                                              │      ├─ printed/pdf/ (private)
                                              │      └─ src/special/ reads
                                              │         content/special/*.meta.yaml
                                              │
                                              ├──→ export-doc/  (docx)   → ATS DOCX
                                              │      └─ printed/ats/  (private)
                                              │
                                              └──→ export-json/  → content.json
                                                     └─ src/lib/locales/{en,it}/
```

Filesystem access goes through `source-nodefs` (`FileSystemSource`), the node
implementation of core-parser's `SourceResolver` seam.

### 2.1 Commands

| Command                          | What it does                                                                                                                                         |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run sync-locales`        | Regenerates every `src/lib/locales/**/cv.ts` from the markdown sources. Run after editing any CV markdown.                                           |
| `npm run pdf:public`          | Builds the 4 public PDFs (en/it main CV, en/it projects) into `public/cv/`. The pre-push hook calls this automatically.                              |
| `npm run pdf -- --input <md>` | Builds one PDF (default printed: `printed/pdf/`). Template is auto-selected by filename or set with `--template main\|projects\|freelance\|special`. |
| `npm run pdf:all`             | Builds every parser-known PDF into `printed/pdf/`. cv-special is excluded (private; explicit `--template special` only).                             |
| `npm run doc -- --input <md>` | Builds one ATS-friendly DOCX. Template auto-selected by filename (`main`, `freelance`, `projects`).                                                  |
| `npm run doc:all`             | Builds every parser-known DOCX into `printed/ats/`. cv-special is excluded (print-only variant).                                                     |

### 2.2 Authoring flow

1. Edit a file under `content/` (markdown or sidecar).
2. Run `npm run pdf:public` (or invoke `pdf` directly for a single
   file) and inspect the resulting PDF.
3. Run `npm run sync-locales` if the site needs to reflect the change.
...
by `export-json/scripts/build.ts`. Do not hand-edit them. Do not
ask an agent to hand-edit them. If something is wrong in the locale,
either:

- the parser has a bug (fix it in `core-parser/src/`), or
...
- The shape of the generated `content.json` is owned by
  `core-parser/src/types.ts` (`ParsedCv`) and `export-json`. Do not add or
  rename fields downstream — change the parser instead.
- The parser output is `ParsedCv { profile, experiences, education, projects,
  footer, keywords }`. `profile` (formerly `cvData`) holds the identity fields;
  `footer` and `keywords` are root-level, not inside `profile`.
- `profile.taglineShort` is always present; when the markdown has no
  short tagline the parser emits `""`.
- `profile.contacts` is `Record<string, string>`.
- `footer` is required; `keywords` defaults to `[]`.
