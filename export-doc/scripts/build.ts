 ...
 *   npm run doc:all                          (every parser-known DOCX → PRINTED_DIR/ats)
 *   npm run doc -- --input <path-to-md> [--template main|freelance|projects] [--out out.docx]
 *
 ...
 *
 * Output:
 *   --out <path>    write to <path> (mirrors export-pdf's --out semantics)
 ...
 * core-parser to obtain a typed payload per template and hands it to the docx
 * renderer in export-doc/src/render.ts. Parse failures abort the build.
 */
...
const source = new FileSystemSource(CONTENT_ROOT, config);
...
async function buildOne(opts: BuildOneOpts): Promise<void> {
  ...
  let doc;
  if (kind === "projects") {
    const projects = await loadParsedProjects(source, mdPath);
...
async function main(): Promise<void> {
  ...
  if (!inputPath) {
    console.error(
      "Usage:\n" +
        "  tsx export-doc/scripts/build.ts --input <path-to-md> [--template main|freelance|projects] [--out printed.docx]\n" +
        "  tsx export-doc/scripts/build.ts --all",
    );
    process.exit(1);
  }
  await buildOne({
