// ATS DOCX renderer.
//
// Pure functions from core-parser types to `docx` Document. No I/O except
// `writeDocx`. Mirrors the export-pdf/templates/ layout conceptually: one entry
...
import type { ParsedCv, Profile, Period, Project } from "core-parser";
 ...
 */
function buildContactLines(cv: Profile, labels: Labels): string[] {
  const lines: string[] = [];
