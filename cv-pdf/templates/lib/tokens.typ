...
// across machines — the build passes --font-path cv-pdf/fonts plus
// --ignore-system-fonts, so only TTFs committed under cv-pdf/fonts/ are
// considered. To swap families: drop the new family under
// cv-pdf/fonts/<Family>/ (any layout — Typst walks recursively) and set
// this constant to its reported family name (run `typst fonts
// --font-path cv-pdf/fonts --ignore-system-fonts` to list them).
//
