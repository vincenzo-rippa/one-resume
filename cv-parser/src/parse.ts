...
import { readProjectsBlock } from "./parseProjects.ts";
...
export interface ParseOptions {
  ...
  file?: string;
  /** Optional sidecar — only `keywords` and per-contact label overrides are honored. */
...
export function parseCv(
  ...
  // Skip leading whitespace/space tokens.
  ...
  const keywords = ctx.sidecar.keywords ?? [];
...
function readName(stream: TokenStream): string {
  ...
  if (token.type !== "heading" || (token as Tokens.Heading).depth !== 1) {
    throw err(stream.ctxRef(), "expected H1 with full name as first line", {
...
function readSelectedTechnologies(stream: TokenStream): string[] {
  ...
  throw err(
    ...
    { line: tokenLine(next, stream.ctxRef().source) },
  );
}
...
function readFooter(stream: TokenStream): string {
  ...
  if (!next) {
    throw err(
      stream.ctxRef(),
      "expected footer paragraph after Education section",
    );
  }
  if (next.type !== "paragraph") {
    throw err(
