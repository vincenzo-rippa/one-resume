...
matching the local toolchain so `/v1/pdf` output is byte-identical to the CLI. The
...
Two things this API intentionally does **not** do — deliberate choices, not
oversights — because it is consumed by a single authenticated static-site build:

- **No response caching.** Every response sends `Cache-Control: no-store`
  (`middlewares/cache.ts`). The content reflects a mutable source and the build
  wants fresh data each run; without an explicit header a 200 GET would be
  _heuristically_ cacheable, so "no header" is not enough.
- **No render concurrency cap.** Each `POST /v1/pdf` spawns a typst process and
  nothing limits how many run at once. The `X-API-KEY` gate is the safeguard: auth
  runs before the controller, so without a valid key no typst is spawned (beyond
  the single `typst --version` boot preflight) — the endpoint can't be used to fan
  out processes.
