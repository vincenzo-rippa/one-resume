...
function makeCtx(query: Record<string, unknown>, files: Record<string, string>): CtxLike {
  return {
    query,
    documentSource: {
      read: async (p: string) => {
...
describe("getContent", () => {
  it("returns the parsed CV with empty projects when only cv is given", async () => {
    ...
    const body = ctx.body as { profile: { name: string }; projects: unknown[]; labels: { projects: string } };
  ...
  it("merges a standalone projects doc — its entries and section label win", async () => {
    ...
    await getContent(ctx as never);
    const body = ctx.body as { projects: { title: string }[]; labels: { projects: string } };
    assert.equal(body.projects.length, 1);
    assert.equal(body.projects[0].title, "Billing Platform");
    assert.equal(body.labels.projects, "Selected Projects");
  });

  it("maps a parse failure to a 422 IoError", async () => {
